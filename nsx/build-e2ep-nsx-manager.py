...
lib.write_to_logs(err, logfile_name)
err = lib.run_terraform_on_pattern_controller(ip_address, config.E2EP_ENVIRONMENT().photonos_username, config.E2EP_ENVIRONMENT().photonos_password, config.NSX().main_tf_git_url, config.NSX().local_py_git_url, config.NSX().run_tf_local_dir, logfile_name)
