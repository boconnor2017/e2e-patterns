...
"variable \"guestinfo.ADMIN_PASSWORD\" { default = \""+config.CLOUD_BUILDER().password+"\" }" ,
"variable \"guestinfo.ADMIN_USERNAME\" { default =  \"admin\" },
"variable \"guestinfo.DNS\"            { default = \""+config.DNS().ip+"\"}",
"variable \"guestinfo.domain\"         { default = \""+config.DNS().zone+"\"}",
"variable \"guestinfo.gateway\"        { default = \""+config.E2EP_ENVIRONMENT().default_gw+"\"}",
"variable \"guestinfo.hostname\"       { default = \""+config.CLOUD_BUILDER().domain_hostname+"\"}",
"variable \"guestinfo.ip0\"            { default = \""+config.CLOUD_BUILDER().ip+"\"}",
"variable \"guestinfo.netmask0\"       { default = \""+config.E2EP_ENVIRONMENT().subnet_mask+"\"}",
"variable \"guestinfo.ntp\"            { default = \""+config.E2EP_ENVIRONMENT().ntp_server+"\"}",
"variable \"guestinfo.ROOT_PASSWORD\"  { default = \""+config.CLOUD_BUILDER().password+"\" }" ,
"variable \"guestinfo.searchpath\"     { default = \""+config.DNS().zone+"\"}",
"variable \"vm.vmname\"                { default = \""+config.CLOUD_BUILDER().domain_hostname+"\"}"
]
